...
class MakeEntityHelper
{
    ...
    public function __construct(DoctrineHelper $doctrineHelper, FileManager $fileManager, EntityClassGenerator $entityClassGenerator, MakeApiResourceHelper $apiResourceHelper)
    {
    ...
    private function createApiFilter($io, $data)
    {
        ...
        while (null === $apiFilter) {
            ...
            $question->setAutocompleterValues($this->apiResourceHelper->getFiltersMatchingCurrentType($data, true));
            $apiFilter = $this->apiResourceHelper->getApiFilterFullClassNameIfExists($io->askQuestion($question));
            ...
            if ('?' === $apiFilter) {
                foreach ($this->apiResourceHelper->getFiltersMatchingCurrentType($data) as $filter) {
            ...
            if (!\in_array($apiFilter, MakeApiResourceHelper::$availableFilters)) {
            ...
            if (!$this->apiResourceHelper->isTypeCompatibleWithApiFilter($data, $apiFilter)) {
            ...
            if ('DateFilter' === $classnameFilter || 'SearchFilter' === $classnameFilter) {
                ...
                while (null === $strategyChoice) {
                    ...
                    $availableStrategies = 'DateFilter' === $classnameFilter
                        ? MakeApiResourceHelper::$availableDateFilterStrategies
                        : MakeApiResourceHelper::$availableSearchFilterStrategies;
