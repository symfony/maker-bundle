...
final class MakeDoctrineListener extends AbstractMaker
{
    ...
    public function generate(InputInterface $input, ConsoleStyle $io, Generator $generator): void
    {
        ...
        if (!$event) {
            throw new RuntimeCommandException('The "event" argument is required in non-interactive mode.');
        }

        if ($entity && !$this->doctrineEventRegistry->isLifecycleEvent($event)) {
        ...
        if ($entity) {
            $this->generateEntityListenerClass($useStatements, $generator, $className, $templateVars, $entity);
        } else {
